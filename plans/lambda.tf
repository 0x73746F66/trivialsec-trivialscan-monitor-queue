...
resource "aws_cloudwatch_event_rule" "monitor_queue_schedule" {
    ...
    name = "${lower(var.app_env)}_monitor_queue_schedule"
...
resource "aws_cloudwatch_event_target" "schedule_lambda" {
    count = var.app_env == "Prod" ? 1 : 0
    rule = one(aws_cloudwatch_event_rule.monitor_queue_schedule[*].name)
    target_id = "${lower(var.app_env)}_trivialscan_monitor_queue"
