...
output: env
	@echo -e $(bold)$(primary)trivialscan_monitor_queue_arn$(clear) = $(shell terraform -chdir=plans output trivialscan_monitor_queue_arn)
	@echo -e $(bold)$(primary)trivialscan_monitor_queue_role$(clear) = $(shell terraform -chdir=plans output trivialscan_monitor_queue_role)
	@echo -e $(bold)$(primary)trivialscan_monitor_queue_role_arn$(clear) = $(shell terraform -chdir=plans output trivialscan_monitor_queue_role_arn)
	@echo -e $(bold)$(primary)trivialscan_monitor_queue_policy_arn$(clear) = $(shell terraform -chdir=plans output trivialscan_monitor_queue_policy_arn)
...
tail-aws-logs: ## Install using pipx install awscliv2
	awsv2 logs tail "/aws/lambda/$(shell sed -e 's/\(.*\)/\L\1/' <<< "${APP_ENV}")-trivialscan-monitor-queue" --follow
