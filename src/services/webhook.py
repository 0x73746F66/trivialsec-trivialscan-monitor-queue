...
def send(event_name: models.WebhookEvent, account: models.MemberAccount, data: dict):
    if account.webhooks.webhook_endpoint and validators.url(account.webhooks.webhook_endpoint) is not True:
    ...
    if getattr(account.webhooks, event_name.value) is True:
        ...
        #TODO sign request and add Authorization header here
        internals.post_beacon(
            url=account.webhooks.webhook_endpoint,
            body=payload.dict(),
        )
